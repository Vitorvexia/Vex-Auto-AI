/**
 * Testes unitários para app/api/internal/reactivation/run/route.ts
 *
 * Coberturas:
 ...
 * - Erro em runReactivationJob → 500
 */
...
const { mockRunReactivationJob } = vi.hoisted(() => ({
...
import { GET, POST } from "@/app/api/internal/reactivation/run/route";
...
const DEFAULT_RESULT = { processed: 2, sent: 2, skipped: 0, failed: 0 };

function makeGetReq(opts: {
  authHeader?: string;
  query?: Record<string, string>;
} = {}): NextRequest {
  const url = new URL("http://localhost/api/internal/reactivation/run");
...
function makePostReq(opts: {
  ...
  return new NextRequest("http://localhost/api/internal/reactivation/run", {
    method: "POST",
    headers,
...
beforeEach(() => {
  ...
  mockRunReactivationJob.mockResolvedValue(DEFAULT_RESULT);
...
describe("GET /api/internal/reactivation/run — autenticação", () => {
...
describe("POST /api/internal/reactivation/run — autenticação", () => {
...
describe("GET — parâmetros via query string", () => {
  it("repassa storeId e limit ao runReactivationJob", async () => {
    await GET(
      makeGetReq({
    ...
    expect(mockRunReactivationJob).toHaveBeenCalledWith({ storeId: "store-42", limit: 10 });
  });

  it("chama runReactivationJob sem opts quando query vazia", async () => {
...
describe("POST — parâmetros via body", () => {
  it("repassa storeId e limit ao runReactivationJob", async () => {
    ...
    expect(mockRunReactivationJob).toHaveBeenCalledWith({ storeId: "store-99", limit: 5 });
  });

  it("chama runReactivationJob sem opts quando body vazio", async () => {
...
describe("Erros de infraestrutura", () => {
  it("GET retorna 500 quando runReactivationJob lança exceção", async () => {
    mockRunReactivationJob.mockRejectedValue(new Error("DB offline"));
    const res = await GET(makeGetReq({ authHeader: `Bearer ${CRON_SECRET}` }));
  ...
  it("POST retorna 500 quando runReactivationJob lança exceção", async () => {
    mockRunReactivationJob.mockRejectedValue(new Error("timeout"));
    const res = await POST(makePostReq({ key: INTERNAL_KEY }));
    expect(res.status).toBe(500);
    const body = await res.json();
    expect(body.ok).toBe(false);
