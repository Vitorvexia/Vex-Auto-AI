...
| 1.7 | 🟡 **IMPLEMENTADO** (2026-08-04) — pendente validação visual do Vitor em produção — **Landing page de vendas do VEX Auto** | Scaffold técnico completo: roteamento, estrutura, captura de lead. Caminho crítico: sem ela não existe onde captar cliente. Investigação confirmou apex/www caíam em `app/page.tsx` → `/inicio` → login, sem landing nenhuma. `isMarketingApexHost` (`lib/subdomain.ts`) + rewrite em `middleware.ts` servem `/marketing` em `"/"` quando Host é apex ou www, reaproveitando `PUBLIC_SITE_ROUTE_HEADER` (mesmo mecanismo do site público de loja, commit `6626f25`) pra não herdar Header do app autenticado. Captura de lead comercial do Vex Auto (não confundir com lead de loja): migration 041 (`demo_requests`, RLS zero-policy, padrão de `audit_logs`), `lib/demo-request.ts` (validação pura) + `lib/demo-request-actions.ts` (Server Action, honeypot reaproveitado de `lib/public-contact-honeypot.ts`). Conteúdo é placeholder — copy e visual final ficam pra ferramenta de design própria do Vitor, fora de escopo deste scaffold. TDD RED→GREEN, 1045 testes unitários verdes (33 novos: 12 em `isMarketingApexHost` + 11 em `demo-request` + 5 em `demo-request-actions` + 5 em `middleware`), lint/typecheck limpos. **Validação de dado real** feita via dev local contra Supabase de produção (não deploy real): honeypot testado, submit real gravou em `demo_requests`, confirmado por query REST com `service_role` e apagado em seguida. **Pendência bloqueante pra fechar o item:** validação visual no domínio real (`vexauto.com.br`) pós-deploy Vercel, só o Vitor confirma — sem isso o item não fecha como ✔ CONCLUÍDO. | — | M |
