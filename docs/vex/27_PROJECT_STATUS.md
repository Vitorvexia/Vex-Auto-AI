...
🟡 BL-0040 — Motor único de mensagens business-initiated (follow-up + reativação), migration 044 + `lib/messaging-eligibility.ts` + `lib/opt-out.ts` (2026-08-26, branch própria `feature/bl-0040-messaging-engine`, baseada em `main` — **não** a branch de redesign visual. 2 commits de código+docs, pushados pra `origin`, PR ainda não aberta).
...
92 testes unitários novos (11 arquivos: `messaging-eligibility`, `opt-out`, `follow-up-m1-m6`, `reactivation-m1-eligibility`, + fixtures atualizadas nos arquivos existentes de follow-up/reativação/ai-pipeline), 1218 testes unitários totais verdes, lint/typecheck limpos. **Pendente antes de fechar**: (1) ~~aplicar migration 044 em produção~~ ✔ feito (2026-08-26, ver Camada 1 abaixo), (2) validar em ambiente real que follow-up e reativação não colidem mais no mesmo lead — em andamento, (3) abrir PR contra `main` (branch só commitada/pushada até agora, sem PR aberta) — mesma disciplina de sempre: código+testes não é "concluído", é "implementado".
...
- Item 4 (cron no painel Vercel): bloqueado até merge — Cron Jobs da Vercel só rodam em deployment de Production, não em preview/branch.
- Item 2 (opt-out, chamada direta + WhatsApp real): pendente — aguardando lead de teste designado pelo founder.
